...
const EXECUTE_DATE: Date = new Date('2022-11-3T20:24:00');
...
function wait(date: Date, fn: Function) {
...
function takeScreenshotAndSave(buf: string | Buffer) {

}

function parseClasses(page: puppeteer.Page) {
    //e
}


function text(number: string, message: string) {

}

function shutdown(code: number = 1) {
    process.exit(code);
}

function debug(log: string, dbl: DEBUG_LEVEL = DEBUG_LEVEL.LOG, sendMessage: boolean = false ) {
...
async function checkInputs() {
    ...
    */
    ...
    try {
        var test = await puppeteer.launch();
        test.close();
    }catch(e) {
